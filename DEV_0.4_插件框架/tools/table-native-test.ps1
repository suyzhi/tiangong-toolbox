...
try{
    ...
    $addin.Object.StartTableTests($output)
    ...
    @(('LIBRARY '+$addin.Object.LoadedLibrary),$result)|Set-Content (Join-Path $output 'table-native.log') -Encoding UTF8
    $result
    if($result -notmatch 'EXIT 0'){throw 'Native table test failed'}
    $blank.Close($false)
}finally{$app.Quit()}
