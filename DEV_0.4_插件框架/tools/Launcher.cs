...
class Launcher {
    [STAThread]static void Main(){Application.EnableVisualStyles();Application.SetCompatibleTextRenderingDefault(false);using(new OleFilter())try{
        ...
        foreach(F.AddIn addin in app.AddIns){if(string.Equals(addin.GUID,"{8C05165C-65A4-4EF2-A138-508589D82004}",StringComparison.OrdinalIgnoreCase)){addin.Connect=true;((dynamic)addin.Object).OpenAutoPanel();return;}}
