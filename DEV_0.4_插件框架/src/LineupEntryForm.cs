...
namespace TianGongCadSuite {
    public sealed partial class LineupTableForm {
        ...
        string entryKind,watchMode="",watchFingerprint="",watchHandled="";
        ...
        void InitializeEntry(){
            ...
            Button(setup,"加入 / 新增 ↵",()=>QuickEntry(false));Button(setup,"连续新增",()=>StartContinuous("add"));
            ...
            KeyDown+=(s,e)=>{if(e.KeyCode==Keys.Escape&&watchMode.Length>0){e.SuppressKeyPress=true;StopContinuous();}if(e.Control&&e.KeyCode==Keys.Enter){e.SuppressKeyPress=true;Try(()=>QuickEntry(false));}if(e.Control&&e.KeyCode==Keys.D){e.SuppressKeyPress=true;Try(FillDown);}};
            ChangeEntryCategory();
        }
        ...
        internal void WriteEntry(LineupEntryPreset preset){
            ...
            entryModel.Text=preset.Model;entryBrand.Text=preset.Brand;entryConnector.Text=preset.Connector;entryDescription.Text=preset.Description;entryMechanism.Text=preset.Mechanism;entryHome.Text=preset.Home;entryMounting.Text=preset.Mounting;entrySensors.Checked=preset.GenerateSensors;entryPosition.SelectedItem=preset.Position;
        ...
        void RefreshEntryParents(string preferred=null){
            ...
            entryParent.Enabled=kind.Length>0;entryParent.AutoCompleteMode=AutoCompleteMode.SuggestAppend;entryParent.AutoCompleteSource=AutoCompleteSource.ListItems;
        }
        void CompleteEntryModel(){
            if(entryLoading||entryModel==null)return;
            var matches=catalog.Match(entryKind,entryModel.Text).ToList();
            if(matches.Count==1){if(entryBrand.Text.Length==0)entryBrand.Text=matches[0].Brand;if(entryConnector.Text.Length==0)entryConnector.Text=matches[0].Connector;}
        ...
        internal int QuickEntry(bool requireSelection){
            ...
            return CommitEntry(incoming,watchMode=="add"?watchPreset:ReadEntry(),requireSelection?1:(int)entryQuantity.Value);
        }
        ...
        internal void StartContinuous(string mode){
            ...
            watchMode=mode;watchFingerprint="";watchHandled="";SetPickingMode(true);watchStop.Visible=true;
        ...
        internal void StopContinuous(){watchMode="";entryWatcher.Stop();watchFingerprint="";watchHandled="";if(watchStop!=null)watchStop.Visible=false;RefreshPickingLabel();}
