...
class Program {
    ...
    [STAThread]static int Main(string[] args){try{Pure();if(args.Length>0&&args[0]=="--cad"){using(new OleFilter())Cad(args.Length>1?args[1]:"artifacts");}Console.WriteLine("CORE ASSERTIONS PASSED "+checks+"; advanced results are listed above.");return 0;}catch(Exception e){Console.Error.WriteLine(e);return 1;}}
