...
namespace TianGongPanelAuto {
    public sealed class AutoPanelForm:Form {
        ...
        public AutoPanelForm(F.Application app,A.AssemblyDocument assembly){
            this.app=app;this.assembly=assembly;Text="多型材自动填充 · DEV 0.2";ClientSize=new Size(1020,660);MinimumSize=new Size(960,660);Font=new Font("Microsoft YaHei UI",9);ShowInTaskbar=false;
        ...
        List<PanelSpec> ReadSpecs(bool refresh){Context();double t,g;if(!double.TryParse(thickness.Text,NumberStyles.Float,CultureInfo.CurrentCulture,out t)||!double.TryParse(gap.Text,NumberStyles.Float,CultureInfo.CurrentCulture,out g))throw new ArgumentException("请输入有效的厚度和间隙（mm）。");
            if(refresh||cachedMembers==null)cachedMembers=FrameReader.Read(assembly,selection);var found=FrameDetection.Detect(cachedMembers,t/1000);var result=found.Select(o=>o.Solve(t/1000,g/1000)).ToList();preview.SourceEdges=cachedMembers.SelectMany(m=>m.Faces).SelectMany(f=>f.Edges).ToList();return result;
        }
