...
namespace TianGongPanelAuto {
    ...
    public static class FrameDetection {
        ...
        public static List<FrameOpening> Detect(IList<FrameMember> members,double thickness){
            ...
            foreach(var m in members){
                ...
                if(length<2*Math.Max(width,b.Z1-b.Z0))throw new ArgumentException("无法可靠确定直型材长轴："+m.Name);
                bounds.Add(b);
            }
            ...
            for(int sx=0;sx<nx;sx++)for(int sy=0;sy<ny;sy++){
                ...
                Support(members,u,v,n,left,bottom,top,depth,thickness);
                Support(members,u,v,n,right,bottom,top,depth,thickness);
                Support(members,v,u,n,bottom,left,right,depth,thickness);
                Support(members,v,u,n,top,left,right,depth,thickness);
                result.Add(new FrameOpening{MidPoint=u*((left+right)/2)+v*((bottom+top)/2)+n*depth,Planes=new[]{new PlaneInput(u*left,u,"自动左边界"),new PlaneInput(u*right,u,"自动右边界"),new PlaneInput(v*bottom,v,"自动下边界"),new PlaneInput(v*top,v,"自动上边界")}});
            }
