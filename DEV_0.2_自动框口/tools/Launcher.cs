...
class Launcher {
    [STAThread]static void Main(){Application.EnableVisualStyles();Application.SetCompatibleTextRenderingDefault(false);using(new OleFilter())try{
        ...
        foreach(F.AddIn addin in app.AddIns){if(string.Equals(addin.GUID,"{B3B28C54-11D0-4488-B226-AD62429CEED2}",StringComparison.OrdinalIgnoreCase)){addin.Connect=true;((dynamic)addin.Object).OpenAutoPanel();return;}}
