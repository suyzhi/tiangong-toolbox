...
class Program {
    ...
    [STAThread]static int Main(string[] args){try{Pure();AutoTests.Pure();if(args.Length>0&&args[0]=="--auto"){using(new OleFilter()){string dir=Path.GetFullPath(Path.Combine(args[1],"auto-"+DateTime.Now.ToString("yyyyMMdd-HHmmss")));Directory.CreateDirectory(dir);AutoTests.Native(Host,dir);}}if(args.Length>0&&args[0]=="--cad"){using(new OleFilter())Cad(args.Length>1?args[1]:"artifacts");}Console.WriteLine("CORE ASSERTIONS PASSED "+checks+"; advanced results are listed above.");return 0;}catch(Exception e){Console.Error.WriteLine(e);return 1;}}
